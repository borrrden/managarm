...
inline void addStringProperty(managarm::mbus::CntRequest<KernelAlloc> &req, const char *name,
...
inline void addIntegralProperty(managarm::mbus::CntRequest<KernelAlloc> &req, const char *name,
    uint64_t value) {
    addStringProperty(req, name, frg::to_allocated_string(*kernelAlloc, value, 10, 0));
}

coroutine<Error> handleRequest(LaneHandle boundLane, int interface, uintptr_t acpi_mmio) {
    ...
    if(!acpi_mmio && interface == 6) {
        acpi_mmio = 0xfed40000;
    } else {
        acpi_mmio = acpi_mmio / 0x1000 * 0x1000;
    }
    ...
    if(preamble.id() == bragi::message_id<managarm::hw::GetMemoryRegionsRequest>) {
        ...
        region.set_tag(frg::string<KernelAlloc>(*kernelAlloc, "locality_0"));
        region.set_type(managarm::hw::IoType::MEMORY);
        region.set_address(acpi_mmio);
        region.set_length(0x1000);
        ...
        if(req->index() > 0) {
            ...
            managarm::hw::SvrResponse<KernelAlloc> resp{*kernelAlloc};
            space = smarter::allocate_shared<HardwareMemory>(*kernelAlloc, acpi_mmio, 0x1000, thor::CachingMode::mmioNonPosted);
            resp.set_error(managarm::hw::Errors::SUCCESS);
        }
...
coroutine<LaneHandle> createObject(LaneHandle mbusLane, unsigned major, unsigned minor) { 
    ...
    addIntegralProperty(req, "version.major", major);
    addIntegralProperty(req, "version.minor", minor);
...
coroutine<void> handleBind(LaneHandle objectLane, int interface, uintptr_t acpi_mmio) {  
    ...
    while(true) {
        ...
        auto error = co_await handleRequest(boundLane, interface, acpi_mmio);
...
static initgraph::Task discoverTpmDevices{&globalInitEngine, "tpm.discover-device",
	...
	[] {
		...
		const char* type = tpm2->platform_class ? "server" : "client";
		if(tpm2->start_method == 6) {
			infoLogger() << "acpi: Found TPM2-TIS " << type << " device" << frg::endlog;
		} else if(tpm2->start_method == 7) {
			infoLogger() << "acpi: Found TPM2-CRB " << type << " device" << frg::endlog;
		} else {
			infoLogger() << "\e[31m" "acpi: Unsupported TPM2 start method " << tpm2->start_method << "\e[39m" << frg::endlog;
			return;
		}
        ...
        KernelFiber::run([=] {
            async::detach_with_allocator(*kernelAlloc, [=]() -> coroutine<void> { 
                ...
                while(true)
                    co_await handleBind(objectLane, tpm2->start_method, tpm2->mmio_area);
            }());
        });
	}	
};
